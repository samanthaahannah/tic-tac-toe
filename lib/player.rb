class Player
  def place_marker(main_board, player, board, endgame, marker)
    if !board.flatten.none?(" ")
      ...
      if input > 0 && input < 4 && board[0][input - 1] == ' '
        ...
        place_marker(main_board, player, board, endgame, marker)
      end
    elsif board.flatten.none?(" ")
      endgame = true
      get_winning_lines(main_board, player, board, endgame, marker)
    end
  end

  def get_winner(main_board, player, board, endgame, marker, draw, player1, player2)
    if player1 == true && player2 == false
      puts 'Player 1 wins!'
      endgame = true
      play_game(main_board, player, board, endgame, marker)
    elsif player1 == false && player2 == true
      puts 'Player 2 wins!'
      endgame = true
      play_game(main_board, player, board, endgame, marker)
    elsif player1 == false && player2 == false && draw == true
      puts "It's a draw"
      endgame = true
      play_game(main_board, player, board, endgame, marker)
    else
  ...
  def get_winning_lines(main_board, player, board, endgame, marker)
    ...
    get_winner(main_board, player, board, endgame, marker, draw, player1, player2)
  end
end